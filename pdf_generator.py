...
def generate_invoice(customer_id, month, year):
    ...
    pdf.cell(30, 10, "Initial Index", border=1)
    pdf.cell(30, 10, "Final Index", border=1)
    pdf.cell(30, 10, "Consumption", border=1)
    pdf.cell(30, 10, "Total Amount", border=1)
    pdf.ln(10)
    pdf.set_font("Arial", "", 12)
    pdf.cell(30, 10,  txt=f"{year}_{month}", border=1)
    pdf.cell(30, 10, str(consumption_details[3])+"RON", border=1)
    pdf.cell(30, 10, str(consumption_details[0])+"kW", border=1)
    pdf.cell(30, 10, str(consumption_details[1])+"kW", border=1)
    pdf.cell(30, 10, str(consumption_details[2])+"kW", border=1)
    pdf.cell(30, 10, str(consumption_details[2]*consumption_details[3])+"RON", border=1)
